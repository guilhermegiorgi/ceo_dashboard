...
const ChatPageCentered: React.FC = () => {
  ...
  const loadAvailableModels = async () => {
    try {
      ...
      if (response.models && response.models.length > 0) {
        ...
        // No models available - show setup message
  ...
  }, [input, isLoading, messages, currentConversationId, selectedModel]);
  ...
  const handleCopy = (text: string, messageId: string) => {
    ...
    setTimeout(() => setCopiedId(messageId), 2000);
  };
      ...
      <div className="flex flex-1 flex-col max-w-4xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between border-b border-zinc-800/50 px-6 py-2">
          <div className="relative">
            <button
              type="button"
              onClick={() => availableModels.length === 0 ? (window.location.href = '/?settings=models') : setShowModelMenu(!showModelMenu)}
