...
class EnhancedChatService {
  ...
  async executeTools(intention, context, workbenchSessionId, options) {
    ...
    switch (intention) {
      ...
      case 'search_brain':
        const searchResult = await mcpClient.call('mcp_semantic_search', {
      ...
      case 'analyze_tasks':
        const taskSummary = await mcpClient.call('mcp_get_tasks_summary', {
          range: 'this_week'
        }, { useCache: true, fallbackLocal: true });
        results.push({
          tool: 'analyze_tasks',
      ...
      case 'generate_insight':
        const graph = await mcpClient.call('mcp_get_graph', {}, { 
          useCache: true, 
          fallbackLocal: true 
        });
        results.push({
